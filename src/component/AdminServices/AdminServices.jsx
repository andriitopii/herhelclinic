...
const ItemEditServiceCurrent = ({data}) =>{
    return(
        <>
        <input
        type="text"
...
const ItemEditService =({data})=>{
    ...
    return(
        <>
        {data ===  null ? <h1>Завантаження</h1> : 
        <form onSubmit={handleSubmit(updateDataService)} className="admin-services__content_form">
            ...
            <div className="admin-services__content_form-row">
                <label>
                PL
                <input type="text" placeholder={data.pl.nameService} {...register("nameServicePL", {required: {value:true}})}/>
                </label>
                <label>
                EN
                <input type="text" placeholder={data.en.nameService } {...register("nameServiceEN", {required: {value:true}})}/>
                </label>
            </div>
            <div className="admin-services__content_form-row">
            ...
            <div className="admin-services__content_form-row">
                <label>
                PL
                <input type="text" placeholder={data.pl.titleService} {...register("titleServicePL", {required: {value:true}})}/>
                </label>
                <label>
                EN
                <input type="text" placeholder={data.en.titleService } {...register("titleServiceEN", {required: {value:true}})}/>
                </label>
            </div>
            <div className="admin-services__content_form-row">
            <h2>Опис послуги</h2>
            ...
            <div className="admin-services__content_form-row">
                <label>
                PL
                <input type="text" placeholder={data.pl.pService} {...register("pServicePL", {required: {value:true}})}/>
                </label>
                <label>
                EN
                <input type="text" placeholder={data.en.pService } {...register("pServiceEN", {required: {value:true}})}/>
                </label>
            </div>
            <div className="admin-services__content_form-row">
            ...
            <div className="admin-services__content_form-row">
                {data.pl.item.map((el)=>{
                    ...
                    for (const key in el) {
                        return(
                            <>
                             <input type="text"  placeholder={Object.keys(el)} {...register(`item${1}PL`, {required: {value:true}})}/>
  ...
  return (
    <div className="admin-services">
      <div className="admin-services__type">
          ...
          <button className="admin-services__type_add-btn" onClick={()=>setAddAndEditService(true)}>Додати послугу</button>
        
        <ul>
          
            
            {dataService.map((item)=> <li key={item.id}><button className={itemId.id === item.id && "admin-active-btn"} onClick={()=>showServiceEdit(item.id)}>{item.pl.nameService}</button></li>)}
